...
func TestMarketReconnect(t *testing.T) {
  ...
  t.Run("PingPong error", func(t *testing.T) {
    ...
    go func (urlChan chan string, msgChan chan string, rootCancel context.CancelFunc, rootWg *sync.WaitGroup) {
      defer rootWg.Done()
      ...
      wg.Wait()
    }(urlChan, msgchan, rootCancel, &rootWg)
